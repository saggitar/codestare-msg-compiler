    ...
    def run(self):
        ...
        if info_file.exists():
            with info_file.open('r') as stream:
                compiled_packages = [l.strip() for l in stream.readlines()]
        else:
            compiled_packages = []
        ...
        current = ['.'.join([self.proto_package, p]) for p in setuptools.find_packages(op.join(self.build_lib,
                                                                                               *proto_package_path))]
        self.announce(f"built protobuf packages: {current}", distutils.log.INFO)

        diff = set(compiled_packages) - set(current)
        if diff:
            outfile = info_file.parent / f"{info_file.name}_last"
            warn(f"compiled protobuf packages changed since last build. writing new packages to {outfile.name}")
